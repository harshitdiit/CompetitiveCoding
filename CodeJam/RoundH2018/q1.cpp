...
void count(TrieNode *root, vector<int> &V, int counter) {
    TrieNode *pCrawl = root;
    ...
    for (int i = 0; i < ALPHABET_SIZE; i++) {
        ...
        count(pCrawl->children[i], V, counter+1);
    }
}
...
int main(){
  ...
  for(int i=1; i<=caseno; i++){
    ...
    BigInt A = pow(Integer(2), N);

    for(int k=0; k<V.size(); k++){
      A -= pow(Integer(2), N-V[k]);
    }
    cout<<"Case #"<<i<<": ";
